...
def get_page(text, pg=0):
    ...
    # Чтобы получить вакансии со страницы, выполняем методом GET http-запрос,
    # включая параметры фильтра поиска (ответ приходит в формате json):
    url = 'https://api.hh.ru/vacancies'
    req = requests.get(url, api_params, headers=get_headers())  # Запрос к API
...
def get_hh_ru_data():
    for page in range(0, 10):
        ...
        # Сохранение файлов страниц в папку {путь до текущего файла}/src/docs/page_files
        # Формирование имени документа (количество файлов в папке - len(...))
        file_name = './src/docs/page_files/{}.json'.format(len(os.listdir('./src/docs/page_files')))
    ...
    for fl in os.listdir('./src/docs/page_files'):
        # Открытие, чтение, закрытие файла
        f = open(f'./src/docs/page_files/{fl}', encoding='utf8')
        json_text = f.read()
        f.close()
        ...
        for vac in json_dict['items']:
            ...
            file_name = f"./src/docs/vacancies/{vac['id']}.json"
